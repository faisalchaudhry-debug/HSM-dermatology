...
import { Mic, Volume2, X, Info, Zap } from 'lucide-react';

interface LipomaVoiceAgentProps {
    onNavigate?: (page: 'cyst' | 'lipoma') => void;
}

const WEBHOOK_URL = 'https://services.leadconnectorhq.com/hooks/Q7V3TZrLpPnz6GYN6Qyt/webhook-trigger/be7b91bc-c243-4f56-8db2-358c97744b29';

const SYSTEM_INSTRUCTION = `You are Sarah, a specialist voice agent for Harley Street Medics London, focusing on Lipoma Removal.

OPENING GREETING: Always start the conversation with: "Hello, this is Sarah from Harley Street Medics. How can I help you today with your lipoma removal concern?"
...
1. Listen to the user's concerns - ask about location, size, duration, and any discomfort.
2. Explain that lipomas are common, benign fatty lumps - not dangerous but can be cosmetically bothersome.
3. Describe both treatment options: Surgical Excision (complete removal) and Lipoma Dissolving Injections (less invasive).
4. Mention our expert: Dr. Mehdi with 20+ years of experience in lipoma removal.
5. Explain that we offer FREE consultations with our specialists for personalized treatment planning.
6. After addressing their concerns, encourage them: "Would you like me to help you book a free consultation with one of our lipoma specialists?"
...
5. CONFIRMATION: After submitting, say: "Perfect! I've submitted your booking request. Our team will contact you within 24 hours to schedule your free lipoma consultation. Is there anything else I can help you with today?"
...
const LipomaVoiceAgent: React.FC<LipomaVoiceAgentProps> = ({ onNavigate }) => {
        ...
        <div className="fixed bottom-8 right-8" style={{ zIndex: 99999, animation: 'fadeIn 0.3s ease-out' }}>
            ...
            {!isActive && !isConnecting ? (
                <button onClick={startSession} className="group relative flex items-center justify-center w-16 h-16 bg-amber-500 gold-gradient rounded-full shadow-2xl hover:scale-110 transition-all">
                    ...
                    <span className="absolute right-full mr-4 bg-black/80 border border-amber-500/30 text-amber-500 text-[10px] font-bold uppercase px-4 py-2 rounded-lg opacity-0 group-hover:opacity-100 whitespace-nowrap">Speak to Lipoma Expert</span>
                </button>
            ) : (
                <div className="bg-neutral-900/95 backdrop-blur-xl border border-amber-500/30 w-80 rounded-2xl shadow-2xl p-6">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center space-x-2"><div className={`w-3 h-3 rounded-full ${isActive ? 'bg-green-500 animate-pulse' : 'bg-amber-500'}`}></div><span className="text-[10px] font-bold uppercase tracking-widest text-amber-500">{isConnecting ? 'Initializing...' : 'Lipoma Expert Agent'}</span></div>
                        <button onClick={stopSession} className="text-neutral-500 hover:text-white"><X size={20} /></button>
                    </div>
                    <div className="flex flex-col items-center text-center space-y-6">
                        <div className="relative"><div className={`w-24 h-24 rounded-full border-2 border-amber-500/30 flex items-center justify-center ${isTalking ? 'scale-110 border-amber-500' : ''}`}><Zap className="text-amber-500 w-10 h-10" /></div>{isActive && <div className="absolute -bottom-2 -right-2 bg-green-500 rounded-full p-1.5 border-4 border-neutral-900"><Volume2 className="text-white w-3 h-3" /></div>}</div>
                        <div><h4 className="text-white font-serif text-lg">{isConnecting ? 'Connecting...' : isTalking ? 'Speaking' : 'Listening...'}</h4><p className="text-neutral-500 text-xs mt-2 italic">Ask about lipoma removal options.</p></div>
...
export default LipomaVoiceAgent;
