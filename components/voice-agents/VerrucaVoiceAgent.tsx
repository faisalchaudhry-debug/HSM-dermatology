...
import { Mic, Volume2, X, Info, Footprints } from 'lucide-react';

interface VerrucaVoiceAgentProps {
    onNavigate?: (page: 'general' | 'verruca' | 'genital') => void;
}

const WEBHOOK_URL = 'https://services.leadconnectorhq.com/hooks/Q7V3TZrLpPnz6GYN6Qyt/webhook-trigger/b7874daf-9057-4220-be5d-d6a2da87146c';

const SYSTEM_INSTRUCTION = `You are Sarah, a foot health specialist voice agent for Harley Street Medics London, focusing on Verruca (Plantar Wart) removal.

OPENING GREETING: Always start the conversation with: "Hello, this is Sarah from Harley Street Medics. How can I help you today with your verruca treatment concern?"
...
- Pricing: Treatment starts from £199.

CONSULTATION GUIDANCE:
1. Listen to the user's concerns about their verrucas (pain level, duration, previous failed treatments).
2. Explain how our Laser Prevention Therapy works - it targets the viral root cause, not just the surface.
3. Emphasize the benefit: walking comfortably again without pain.
4. Explain that we offer FREE consultations with our foot health specialists.
5. After addressing their concerns, encourage them: "Would you like me to help you book a free consultation with one of our specialists?"
...
5. CONFIRMATION: After submitting, say: "Perfect! I've submitted your booking request. Our team will contact you within 24 hours to schedule your free verruca consultation. Is there anything else I can help you with today?"
...
const VerrucaVoiceAgent: React.FC<VerrucaVoiceAgentProps> = ({ onNavigate }) => {
                        ...
                        functionDeclarations: [
                            {
                                ...
                                parameters: { type: Type.OBJECT, properties: { sectionId: { type: Type.STRING, enum: ['treatments', 'gallery', 'reviews', 'financing', 'team', 'faq', 'booking-section'] } }, required: ['sectionId'] }
                            },
                            {
                                name: 'navigateToPage',
                                description: 'Navigates to a different treatment page.',
                                parameters: { type: Type.OBJECT, properties: { pageId: { type: Type.STRING, enum: ['general', 'verruca', 'genital'] } }, required: ['pageId'] }
                            },
                            {
                                name: 'submitLeadToWebhook',
                                description: 'Submits the collected lead information (name, email, phone) to book a consultation.',
                                parameters: { type: Type.OBJECT, properties: { name: { type: Type.STRING }, email: { type: Type.STRING }, phone: { type: Type.STRING } }, required: ['name', 'email', 'phone'] }
                            }
                        ]
                    }]
                },
                callbacks: {
                    ...
                    onmessage: async (message) => {
                        ...
                        const executeScroll = (sectionId: string, callId?: string) => {
                            ...
                            sessionPromise.then((s) => { try { s.sendToolResponse({ functionResponses: [{ id: callId, name: 'scrollToSection', response: { success: !!element } }] }); } catch { } });
                        };
                        ...
                        const executeWebhook = async (name: string, email: string, phone: string, callId?: string) => {
                            console.log('VerrucaVoiceAgent: Submitting lead:', { name, email, phone });
                            try {
                                const response = await fetch(WEBHOOK_URL, {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ name, email, phone, source: 'Voice Agent - Verruca Removal', timestamp: new Date().toISOString() })
                                });
    ...
    return (
        <div className="fixed bottom-8 right-8" style={{ zIndex: 99999, animation: 'fadeIn 0.3s ease-out' }}>
            ...
            {!isActive && !isConnecting ? (
                <button onClick={startSession} className="group relative flex items-center justify-center w-16 h-16 bg-amber-500 gold-gradient rounded-full shadow-2xl hover:scale-110 transition-all duration-300">
                ...
                <div className="bg-neutral-900/95 backdrop-blur-xl border border-amber-500/30 w-80 rounded-2xl shadow-2xl p-6">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-widest text-amber-500">{isConnecting ? 'Initializing...' : 'Verruca Agent'}</span>
                        </div>
                    ...
                    <div className="flex flex-col items-center text-center space-y-6">
                        <div className="relative">
                            <div className={`w-24 h-24 rounded-full border-2 border-amber-500/30 flex items-center justify-center ${isTalking ? 'scale-110 border-amber-500' : ''}`}>
                                <Footprints className="text-amber-500 w-10 h-10" />
                            </div>
                        ...
                        <div>
                            ...
                            <p className="text-neutral-500 text-xs mt-2 italic">Ask about verruca treatments.</p>
                        </div>
                        {error && <div className="bg-red-500/10 border border-red-500/30 text-red-500 text-[10px] p-3 rounded-lg flex items-center space-x-2"><Info size={14} /><span>{error}</span></div>}
                        <button onClick={stopSession} className="text-[10px] uppercase tracking-widest text-neutral-500 font-bold hover:text-amber-500">End Conversation</button>
                    </div>
                </div>
            )}
        </div>
...
export default VerrucaVoiceAgent;
