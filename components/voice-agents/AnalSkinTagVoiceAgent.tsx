...
interface AnalSkinTagVoiceAgentProps {
    onNavigate?: (page: 'skintag' | 'analskintag' | 'mole') => void;
}

const WEBHOOK_URL = 'https://services.leadconnectorhq.com/hooks/Q7V3TZrLpPnz6GYN6Qyt/webhook-trigger/de0b4ae1-b049-41bf-92d4-474f15e25b12';

const SYSTEM_INSTRUCTION = `You are Sarah, a specialist medical voice agent for Harley Street Medics London, focusing on Anal Skin Tag removal.

OPENING GREETING: Always start the conversation with: "Hello, this is Sarah from Harley Street Medics. How can I help you today with your anal skin tag treatment concern? Rest assured, your conversation is completely confidential."
...
1. Listen with empathy - many patients feel embarrassed discussing this, so be reassuring.
2. Explain that anal skin tags are very common and our specialists handle these procedures daily with complete discretion.
3. Describe the Signature Laser Treatment - precise, painless, no downtime, performed in a private clinical setting.
4. Emphasize complete confidentiality and professional, judgment-free care.
...
const AnalSkinTagVoiceAgent: React.FC<AnalSkinTagVoiceAgentProps> = ({ onNavigate }) => {
        ...
        <div className="fixed bottom-8 right-8" style={{ zIndex: 99999, animation: 'fadeIn 0.3s ease-out' }}>
            ...
            {!isActive && !isConnecting ? (
                <button onClick={startSession} className="group relative flex items-center justify-center w-16 h-16 bg-amber-500 gold-gradient rounded-full shadow-2xl hover:scale-110 transition-all">
                    ...
                    <span className="absolute right-full mr-4 bg-black/80 border border-amber-500/30 text-amber-500 text-[10px] font-bold uppercase px-4 py-2 rounded-lg opacity-0 group-hover:opacity-100 whitespace-nowrap">Confidential Consultation</span>
                </button>
            ) : (
                <div className="bg-neutral-900/95 backdrop-blur-xl border border-amber-500/30 w-80 rounded-2xl shadow-2xl p-6">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center space-x-2"><div className={`w-3 h-3 rounded-full ${isActive ? 'bg-green-500 animate-pulse' : 'bg-amber-500'}`}></div><span className="text-[10px] font-bold uppercase tracking-widest text-amber-500">{isConnecting ? 'Initializing...' : 'Confidential Agent'}</span></div>
                        <button onClick={stopSession} className="text-neutral-500 hover:text-white"><X size={20} /></button>
                    </div>
                    <div className="flex flex-col items-center text-center space-y-6">
                        <div className="relative"><div className={`w-24 h-24 rounded-full border-2 border-amber-500/30 flex items-center justify-center ${isTalking ? 'scale-110 border-amber-500' : ''}`}><Lock className="text-amber-500 w-10 h-10" /></div>{isActive && <div className="absolute -bottom-2 -right-2 bg-green-500 rounded-full p-1.5 border-4 border-neutral-900"><Volume2 className="text-white w-3 h-3" /></div>}</div>
                        <div><h4 className="text-white font-serif text-lg">{isConnecting ? 'Connecting...' : isTalking ? 'Speaking' : 'Listening...'}</h4><p className="text-neutral-500 text-xs mt-2 italic">Your conversation is completely confidential.</p></div>
...
export default AnalSkinTagVoiceAgent;
