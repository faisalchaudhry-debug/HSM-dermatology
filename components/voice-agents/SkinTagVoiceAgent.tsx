...
import { Mic, Volume2, X, Info, Tag } from 'lucide-react';

interface SkinTagVoiceAgentProps {
    onNavigate?: (page: 'skintag' | 'analskintag' | 'mole') => void;
}

const WEBHOOK_URL = 'https://services.leadconnectorhq.com/hooks/Q7V3TZrLpPnz6GYN6Qyt/webhook-trigger/e40236e4-67ea-4590-84dd-8fb375feeeed';

const SYSTEM_INSTRUCTION = `You are Sarah, a cosmetic dermatology specialist voice agent for Harley Street Medics London, focusing on Skin Tag removal.

OPENING GREETING: Always start the conversation with: "Hello, this is Sarah from Harley Street Medics. How can I help you today with your skin tag removal concern?"
...
1. Listen to the user's concerns about their skin tags (location, number, cosmetic impact).
2. Explain our Signature Laser Treatment - quick procedure, painless, no downtime, scar-free results.
3. Discuss how skin tags can be removed in a single session with immediate aesthetic improvement.
4. Explain that we offer FREE consultations with our cosmetic dermatologists.
5. After addressing their concerns, encourage them: "Would you like me to help you book a free consultation with one of our cosmetic specialists?"
...
5. CONFIRMATION: After submitting, say: "Perfect! I've submitted your booking request. Our team will contact you within 24 hours to schedule your free skin tag consultation. Is there anything else I can help you with today?"
...
const SkinTagVoiceAgent: React.FC<SkinTagVoiceAgentProps> = ({ onNavigate }) => {
        ...
        <div className="fixed bottom-8 right-8" style={{ zIndex: 99999, animation: 'fadeIn 0.3s ease-out' }}>
            ...
            {!isActive && !isConnecting ? (
                <button onClick={startSession} className="group relative flex items-center justify-center w-16 h-16 bg-amber-500 gold-gradient rounded-full shadow-2xl hover:scale-110 transition-all">
                    ...
                    <span className="absolute right-full mr-4 bg-black/80 border border-amber-500/30 text-amber-500 text-[10px] font-bold uppercase px-4 py-2 rounded-lg opacity-0 group-hover:opacity-100 whitespace-nowrap">Speak to Skin Tag Expert</span>
                </button>
            ) : (
                <div className="bg-neutral-900/95 backdrop-blur-xl border border-amber-500/30 w-80 rounded-2xl shadow-2xl p-6">
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center space-x-2"><div className={`w-3 h-3 rounded-full ${isActive ? 'bg-green-500 animate-pulse' : 'bg-amber-500'}`}></div><span className="text-[10px] font-bold uppercase tracking-widest text-amber-500">{isConnecting ? 'Initializing...' : 'Skin Tag Agent'}</span></div>
                        <button onClick={stopSession} className="text-neutral-500 hover:text-white"><X size={20} /></button>
                    </div>
                    <div className="flex flex-col items-center text-center space-y-6">
                        <div className="relative"><div className={`w-24 h-24 rounded-full border-2 border-amber-500/30 flex items-center justify-center ${isTalking ? 'scale-110 border-amber-500' : ''}`}><Tag className="text-amber-500 w-10 h-10" /></div>{isActive && <div className="absolute -bottom-2 -right-2 bg-green-500 rounded-full p-1.5 border-4 border-neutral-900"><Volume2 className="text-white w-3 h-3" /></div>}</div>
                        <div><h4 className="text-white font-serif text-lg">{isConnecting ? 'Connecting...' : isTalking ? 'Speaking' : 'Listening...'}</h4><p className="text-neutral-500 text-xs mt-2 italic">Ask about skin tag removal.</p></div>
...
export default SkinTagVoiceAgent;
